...
\caption{Conceptual architecture of TAKT Governed Runtime $M$ enforcing decision preservation $\pi^*(R)$ under state abstraction $\rho(S)$.}
\label{fig:architecture}
\end{figure}

\textbf{Definition (Capability Necessity):} A capability $C \in \mathcal{C}$ is \textit{locally necessary} for runtime $M$ if removing $C$ alters policy decisions on at least one representation $R$:
$$\exists R \in \mathcal{R}: \pi_M(R) \neq \pi_{M \setminus \{C\}}(R)$$

\textbf{Example 1 (Temporal Prefix Necessity):} Consider a runtime where temporal prefix monitoring ($C_{\text{temporal}}$) is ablated. Two trajectories $\tau_1 = (r_0, r_1, r_2)$ and $\tau_2 = (r'_0, r'_1, r_2)$ share an identical terminal state $r_2$. If $\tau_1$ represents a safe approach while $\tau_2$ represents an unsafe trajectory, an ungoverned runtime inspecting only $r_2$ executes identical actions, causing decision divergence ($\pi_M(\tau_1) \neq \pi^*(\tau_1)$).
