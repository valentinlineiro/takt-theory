...
an optimal policy $\pi^*_S : \mathcal{S} \to \mathcal{D}$ computed on full observable
state $S$ may fail when the governed runtime's policy $\pi_M : \mathcal{R} \to \mathcal{D}$
is evaluated on compressed representation $R$.
...
\begin{enumerate}
    ...
    \item \textbf{Formalized Witness Scenarios} \emph{(within the formal ST-016 model):}
    We construct Lean 4 certified witness scenarios demonstrating that removing any of
    $\{C_{\text{contract}}, C_{\text{uncertainty}}, C_{\text{temporal}}\}$ from the
    formal model produces decision preservation violations for specific representations.
    \item \textbf{Formal Witness Validation Schema:} We introduce a structured schema
    connecting empirical ablation observations (EXP-004) to the formal model:
    witness artifacts are manually constructed from TypeScript runtime traces and
    formally verified against the Lean model via \texttt{WitnessConsistentWithRuntime}.
