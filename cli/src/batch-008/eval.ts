...
import type { CorruptionState } from './sparseFn.js';

export type PolicyId = 'AlwaysTrust' | 'AlwaysVerify' | 'ProbeAlways' | 'ProbeReliability0.3' | 'ProbeReliability0.5' | 'ProbeReliability0.7';
...
export function evaluateOne(
  ...
  const pols: { id: PolicyId; threshold?: number }[] = [
    { id: 'AlwaysTrust' },
    { id: 'AlwaysVerify' },
    { id: 'ProbeAlways' },
  ...
  for (const p of pols) {
    ...
    if (p.id === 'AlwaysTrust') {
      ...
      if (corruptStopK < corruptCd.kMax) {
        ...
        kActual = corruptStopK;
        chosenAction = pickBest(corruptCd, buildObs(corruptCd, extractObservableSubgraph(corruptGraph, corruptCd.focalElement, kActual)));
      }
    } else {
      const threshold = p.threshold ?? 1;
      if (corruptStopK < corruptCd.kMax && alphaEstimated > threshold) {
        didProbe = true;
        actionCost = C_EXPAND;
        kActual = corruptStopK + 1;
        const O = extractObservableSubgraph(corruptGraph, corruptCd.focalElement, kActual);
    ...
    const alphaError = alphaTrue - alphaEstimated;
