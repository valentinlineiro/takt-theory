...
export function runBatch008(): ScoreRow[] {
  const all: ScoreRow[] = [];
  ...
  for (const caseId of CASE_IDS) {
    ...
    for (const incidence of INCIDENCES) {
      for (let rep = 1; rep <= REPS; rep++) {
        ...
        const rows = evaluateOne(orig, cs, incidence, rep);
        all.push(...rows);
      }
    }
  }
...
export function aggregate(results: ScoreRow[]): Record<string, any> {
  ...
  for (const [policyId, rows] of Object.entries(groups)) {
    const n = rows.length;
    out[policyId] = {
      n,
      ...
      optimalCost: rows[0].optimalActionCost,
      avgAlphaEst: rows.reduce((s, r) => s + r.alphaEstimated, 0) / n,
      avgAlphaTrue: rows.reduce((s, r) => s + r.alphaTrue, 0) / n,
    };
  }
  return out;
}
