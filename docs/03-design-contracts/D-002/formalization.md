...
El stress-test adversarial [ST-004 (Hidden Kernel Attack)](../../../experiments/stress-tests/ST-004/conclusion.md) demostró que validar la seguridad decisional de una representación sobre un conjunto de test local $T \subset S$ no es un certificado de seguridad global ($\text{safe}_T(R) \not\implies \text{safe}_S(R)$). Esto ocurre porque el test de validación puede dejar partes del dominio decisional o colisiones del kernel sin observar.
...
El teorema fundamental de generalización y la verificación del contraejemplo de ST-004 han sido implementados y probados en [Coverage.lean](../../../takt-formal/TaktFormal/Coverage.lean):
